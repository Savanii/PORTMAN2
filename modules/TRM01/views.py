...
bp = Blueprint('TRM01', __name__, template_folder='.')
MODULE_CODE = 'TRM01'
...
@bp.route('/module/TRM01/')
@login_required
def view():
    perms = get_perms()
    if not perms.get('can_read'):
        return render_template('no_access.html'), 403
    return render_template('trm01.html', permissions=perms)

@bp.route('/api/module/TRM01/data')
@login_required
def get_data():
...
@bp.route('/api/module/TRM01/all')
@login_required
def get_all():
    return jsonify([r['terminal_name'] for r in model.get_all_active()])

@bp.route('/api/module/TRM01/save', methods=['POST'])
@login_required
def save():
    perms = get_perms()
    data = request.json
...
@bp.route('/api/module/TRM01/delete', methods=['POST'])
@login_required
def delete():
    perms = get_perms()
